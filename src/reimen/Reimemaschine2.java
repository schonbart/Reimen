...
public class Reimemaschine2 extends JFrame {
	...
	public Reimemaschine2() {
		...
		btnReim.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				/*
				 * Ich habe einiges ver√§ndert (wie man sieht :D)
				 * 	Ich habe das Suchen nach Vokalen in einem String in eine extra Methode ausgelagert.
				 *	Du hattest den gleichen Code mehrmals stehen, was redundant war. Durch die Auslagerung
				 *	in eine extra-Methode ist der Code weniger redundant und kann flexibler ge√§ndert werden.
				 * 
				 * 	Das Durchgehen durch eine Datei (oder eines Strings) mit einem Scanner geht vorzugsweise
				 * 	in einer while-Schleife. Die Methode hasNextLine() fungiert hier als Abbruchsbedingung.
				 * 	Beispiele finden sich reihenweise bei Google.
				 * 
				 * 	Den Kontrollfluss eines Programms sollte man generell nicht mit einkalkulierten 
				 * 	Fehlern modellieren. Deshalb hat der Scanner beispielsweise so welche Methoden, um
				 * 	einer Exception vorzubeugen.
				 * 
				 * 	Dazu habe ich hier ein paar lokale Variablen umbenannt, da ich so einen besseren 
				 * 	√úberblick habe. Das ist aber wohl Meinungssache.
				 *		
				 */
	...
	/**
	 * Geht durch einen String durch und gibt in der
	 * Reihenfolge, in der es die Vokale darin findet genau diese zur√ºck.
	 * 
	 * 
	 * K√∂nnte noch so modifiziert werden, dass nicht nur einzelne Vokale, 
	 * sondern z.B. auch Sachen wie "eu" und so weiter extra behandelt werden.
	 * Das √ºberlasse ich dann dir ;)
	 * 
	...
	private String vokaleVonString(String s) {
		...
		for (int i = 0; i < sLowerCase.length(); i++) {
			if (gueltigeVokale.contains(String.valueOf(s.charAt(i)))) {
				
				// jetzt wirds kompliziert :) -> wie die ganzen if Anweisungen durch eine weitere Methode ersetzen?
				if (s.charAt(i) == 'e') {
					if (i+1 < sLowerCase.length()) {// gibt es ¸berhaupt noch einen Buchstaben danach?
						...
						if (s.charAt(i+1) == 'i') {// wenn e und danach i im zu traversierenden String stehen
							vokale.append('a');// steht ein a in dem Vokale StringBuilder, dann steht das stellvertretend f¸r ei -> weil 'a' reimt sich ja auch auf 'ei'
							i++;// einen weitergehen, damit wir nicht zus‰tzlich das i nach dem e noch als Vokal ansehen
						} else { vokale.append(s.charAt(i));}
					} else { vokale.append(s.charAt(i));}
				}
				...
				} else { vokale.append(s.charAt(i));}// ganz am Ende das else f¸r wenn wir alle ifs durchhaben (also alle Spezialf‰lle bei bestimmten Buchstaben)
